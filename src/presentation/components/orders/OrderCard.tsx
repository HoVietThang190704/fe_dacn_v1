...
export const OrderCard: React.FC<{ order: Order; onViewDetail: () => void; onCancel: () => void; locale: string }> = ({ order, onViewDetail, onCancel, locale }) => {
  ...
  const cancelReason = order.cancelReason ? translateWithValues(t, 'labels.cancelReason', { reason: order.cancelReason }, order.cancelReason) : undefined;
      ...
      <div className="flex flex-col gap-4 px-4 py-4 sm:px-6 sm:py-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-1 items-center gap-4">
          <div className={`relative overflow-hidden rounded-xl bg-gray-100 ${ORDER_CONFIG.IMAGE_SIZE_CLASS}`}>
            ...
            {remainingItems > 0 && (
              <span className="absolute bottom-1 right-1 rounded-full bg-black/70 px-2 py-0.5 text-[11px] font-semibold text-white">+{remainingItems}</span>
            )}
          </div>
          <div className="min-w-0 flex-1">
            <p className="line-clamp-2 text-sm font-medium text-gray-900 sm:text-base">{leadItem?.productName ?? translateSafely(t, 'labels.noProduct', 'Sản phẩm không xác định')}</p>
            <p className="mt-1 text-xs text-gray-500 sm:text-sm">
              {leadItem ? `x${leadItem.quantity}` : ''}
              {remainingItems > 0 && ` · ${order.totalItems} ${t('amountLabel').toLowerCase() || 'sản phẩm'}`}
            </p>
        ...
        <div className="flex flex-none flex-col items-start gap-2 text-sm text-gray-500 sm:items-end">
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            <button onClick={onViewDetail} className="inline-flex items-center rounded-full bg-orange-500 px-4 py-2 text-xs font-medium text-white transition-colors hover:bg-orange-600 sm:text-sm">{translateSafely(t, 'actions.details', 'Xem chi tiết')}</button>
            {order.canBeCancelled && cancellableStatuses.includes(order.status) && (
              <button onClick={onCancel} className="inline-flex items-center rounded-full border border-gray-300 px-4 py-2 text-xs font-medium text-gray-600 transition-colors hover:bg-gray-100 sm:text-sm">{translateSafely(t, 'actions.cancel', 'Hủy đơn')}</button>
            )}
          </div>
        </div>
      </div>
